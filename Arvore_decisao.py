...
numero_transacoes = dados['Class'].count()
transacoes_fraudulentas = dados['Class'].sum()
transacoes_normais = numero_transacoes - transacoes_fraudulentas

porcentagem_fraudes = transacoes_fraudulentas / numero_transacoes
porcentagem_normal = transacoes_normais / numero_transacoes

print('Numero de transacoes : ', numero_transacoes)
print('Transacoes fraudulentas:', transacoes_fraudulentas, "%.2f" % (porcentagem_fraudes * 100))
print('Transacoes normais : ', transacoes_normais, "%.2f" %(porcentagem_normal * 100))
...
#validando a acuracia da arvore
#validar_arvore(y_test, y_pred_arvore_decisao)

#Profundidade da arvore de decisão, terá tamanho 10
classificador_arvore_decisao = tree.DecisionTreeClassifier(max_depth=10, random_state=0)
y_pred_arvore_decisao = executar_classificador(classificador_arvore_decisao, x_train, x_test, y_train)
